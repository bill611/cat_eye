...
#include "ctrlmisc.h"
// #include "scrolled.h"
#include "my_scrollview_impl.h"

#define CTRL_NAME CTRL_MYSCROLLVIEW
...
void scrollview_draw (HWND hWnd, HDC hdc, PSVDATA psvdata)
{
    ...
    PSVLIST psvlist = &psvdata->svlist;
#define PRINT_RC(rc) \
	printf("line:%d,l:%d,t:%d,r:%d,b:%d\n",__LINE__,rc.left,rc.top,rc.right,rc.bottom );
...
static BOOL myScrollViewRegist (void)
{
    WNDCLASS WndClass;
...
static void myScrollViewCleanUp (void)
{
...
{
	my_scrollview = (MyControls *)malloc(sizeof(MyControls));
	my_scrollview->regist = myScrollViewRegist;
	my_scrollview->unregist = myScrollViewCleanUp;
	my_scrollview->bmpsLoad = NULL;
	my_scrollview->bmpsRelease = NULL;
}
