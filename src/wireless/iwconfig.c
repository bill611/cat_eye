...
// #define TC_DEBUG
#ifdef TC_DEBUG
	#define IWCONFIG_INFO(...)  printf(__VA_ARGS__)
#else
	#define IWCONFIG_INFO(...)  
#endif
	     ...
	     char *			ifname)
{
  ...
  IWCONFIG_INFO("%-8.16s  %s  ", ifname, info->b.name);
  ...
  if(info->b.has_essid)
      ...
      if(info->b.essid_on)
	{
	  ...
	  if((info->b.essid_on & IW_ENCODE_INDEX) > 1)
	    IWCONFIG_INFO("ESSID:\"%s\" [%d]  ", info->b.essid,
		   (info->b.essid_on & IW_ENCODE_INDEX));
	  else
	    IWCONFIG_INFO("ESSID:\"%s\"  ", info->b.essid);
	}
      else
	IWCONFIG_INFO("ESSID:off/any  ");
    }

#ifndef WE_ESSENTIAL
  /* Display NickName (station name), if any */
  if(info->has_nickname)
    IWCONFIG_INFO("Nickname:\"%s\"", info->nickname);
#endif	/* WE_ESSENTIAL */
  ...
  if(info->b.has_nwid)
	...
	IWCONFIG_INFO("NWID:off/any  ");
      else
	IWCONFIG_INFO("NWID:%X  ", info->b.nwid.value);
      tokens +=2;
    }
#endif	/* WE_ESSENTIAL */
  ...
  if(info->b.has_mode)
    {
      IWCONFIG_INFO("Mode:%s  ", iw_operation_mode[info->b.mode]);
      tokens +=3;
    }
  ...
  if(info->has_bitrate)
      ...
      if(tokens > 11)
	{
      ...
      IWCONFIG_INFO("Bit Rate%c%s   ", (info->bitrate.fixed ? '=' : ':'), buffer);
    }
  ...
  if(info->has_txpower)
      ...
      if(tokens > 11)
	{
      ...
      IWCONFIG_INFO("Tx-Power%c%s   ", (info->txpower.fixed ? '=' : ':'), buffer);
    }
  ...
  if(info->has_sens)
      ...
      if(tokens > 10)
	{
      ...
      /* Fixed ? */
      IWCONFIG_INFO("Sensitivity%c", info->sens.fixed ? '=' : ':');

      if(info->has_range)
	/* Display in dBm ? */
	if(info->sens.value < 0)
	  IWCONFIG_INFO("%d dBm  ", info->sens.value);
	else
	  IWCONFIG_INFO("%d/%d  ", info->sens.value, info->range.sensitivity);
      else
	IWCONFIG_INFO("%d  ", info->sens.value);
    }
#endif	/* WE_ESSENTIAL */
  ...
  if(info->has_rts)
	...
	IWCONFIG_INFO("RTS thr:off   ");
      else
	{
	  /* Fixed ? */
	  IWCONFIG_INFO("RTS thr%c%d B   ",
		 info->rts.fixed ? '=' : ':',
		 info->rts.value);
	}
      tokens += 3;
  ...
  if(info->has_frag)
	...
	IWCONFIG_INFO("Fragment thr:off");
      else
	{
	  /* Fixed ? */
	  IWCONFIG_INFO("Fragment thr%c%d B   ",
		 info->frag.fixed ? '=' : ':',
		 info->frag.value);
	}
    }
  ...
  if(info->b.has_key)
      ...
      else
	{
	  ...
	  if((info->b.key_flags & IW_ENCODE_INDEX) > 1)
	    IWCONFIG_INFO(" [%d]", info->b.key_flags & IW_ENCODE_INDEX);
	  if(info->b.key_flags & IW_ENCODE_RESTRICTED)
	    IWCONFIG_INFO("   Security mode:restricted");
	  if(info->b.key_flags & IW_ENCODE_OPEN)
	    IWCONFIG_INFO("   Security mode:open");
 	}
      IWCONFIG_INFO("\n          ");
    }
  ...
  if(info->has_stats)
    {
      ...
      IWCONFIG_INFO("Link %s\n", buffer);

      if(info->range.we_version_compiled > 11)
	IWCONFIG_INFO("          Rx invalid nwid:%d  Rx invalid crypt:%d  Rx invalid frag:%d\n          Tx excessive retries:%d  Invalid misc:%d   Missed beacon:%d\n",
	...
	IWCONFIG_INFO("          Rx invalid nwid:%d  invalid crypt:%d  invalid misc:%d\n",
	       info->stats.discard.nwid,
	       info->stats.discard.code,
	       info->stats.discard.misc);
    }
